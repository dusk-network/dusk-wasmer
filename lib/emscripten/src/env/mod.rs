...
pub fn ___build_environment(mut ctx: ContextMut<'_, EmEnv, ()>, environ: c_int) {
    ...
    let environment = emscripten_memory_pointer!(ctx, ctx.state().memory(0), environ) as *mut c_int;
    let (mut pool_offset, env_ptr, mut pool_ptr) = unsafe {
        ...
        let env_ptr =
            emscripten_memory_pointer!(ctx, ctx.state().memory(0), env_offset) as *mut c_int;
        let pool_ptr =
            emscripten_memory_pointer!(ctx, ctx.state().memory(0), pool_offset) as *mut u8;
...
pub fn _pathconf(ctx: ContextMut<'_, EmEnv, ()>, path_addr: c_int, name: c_int) -> c_int {
    ...
    let _path = emscripten_memory_pointer!(ctx, ctx.state().memory(0), path_addr) as *const c_char;
    match name {
        0 => 32000,
        1 | 2 | 3 => 255,
        4 | 5 | 16 | 17 | 18 => 4096,
