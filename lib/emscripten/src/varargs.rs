...
impl VarArgs {
    pub fn get<T: Sized>(&mut self, ctx: &ContextMut<'_, EmEnv, ()>) -> T {
        let ptr = emscripten_memory_pointer!(ctx, ctx.state().memory(0), self.pointer);
    ...
    pub fn get_str(&mut self, ctx: &ContextMut<'_, EmEnv, ()>) -> *const c_char {
        ...
        let ptr = emscripten_memory_pointer!(ctx, ctx.state().memory(0), ptr_addr) as *const c_char;
        ptr
